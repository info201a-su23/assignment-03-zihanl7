...
most_recent_date <- max(national$date)

recent_data <- national %>%
  filter(date == most_recent_date)

total_us_cases <- pull(recent_data, cases)
...
total_us_deaths <- pull(recent_data, deaths)
...
state_highest_cases <- state_cases %>%
...
state_lowest_cases <- state_cases %>%
...
county_highest_cases <- counties %>%
  filter(cases == num_highest_cases_county) %>%
  pull(county)
...
#What is the average number of new cases per day in the United States during the most recent time period available in the dataset?
